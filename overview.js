...
const meta=[['vienna','항공 · 도착','VIENNA','plane'],['vienna','도보 · 문화','VIENNA','walk'],['lakes','렌터카 · 이동','VIENNA → SALZBURG','car'],['lakes','렌터카 · 도보','SALZBURG','walk'],['lakes','렌터카 · 호수','SALZKAMMERGUT','car'],['prague','반납 → 기차','LINZ → PRAGUE','train'],['prague','도보 · 구시가지','PRAGUE','walk'],['prague','산책 · 자유시간','PRAGUE','walk'],['prague','항공 · 귀국','PRAGUE → INCHEON','plane']];
...
function render(){window.dispatchEvent(new CustomEvent('journey-schedule',{detail:days.map((d,i)=>({title:d.title,date:d.date,rows:current[i]}))}));const focused=document.activeElement?.closest?.('.day-card')?.getAttribute('href');grid.innerHTML=renderCards();if(focused)Array.from(grid.querySelectorAll('a')).find(a=>a.getAttribute('href')===focused)?.focus({preventScroll:true})}
